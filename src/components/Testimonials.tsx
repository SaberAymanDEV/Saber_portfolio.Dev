    ...
    >
      <div className="max-w-7xl mx-auto px-4 md:px-8">
        ...
        {testimonials.length === 0 ? (
          <div className="text-center py-16 bg-white dark:bg-slate-950 border border-slate-200 dark:border-slate-900 rounded-3xl max-w-4xl mx-auto px-4 md:px-8 shadow-md">
            <MessageSquare className="h-10 w-10 mx-auto mb-4 text-blue-500/60 animate-bounce" />
            <p className="text-sm font-semibold text-slate-700 dark:text-slate-300">
              {lang === "ar" ? "أضف توصياتك الأولى عبر لوحة التحكم" : "Ready to receive testimonials! Add them via the Admin panel."}
            </p>
          </div>
        ) : (
              ...
              <div className="space-y-6 relative z-10">
                ...
                <div className="flex items-center gap-4 pt-4 border-t border-slate-100 dark:border-slate-900">
                  <div className="h-11 w-11 flex items-center justify-center rounded-xl bg-gradient-to-br from-blue-600 to-indigo-600 text-white font-extrabold text-sm tracking-tight">
                    {(testimonials[activeIndex]?.name || "Anonymous").split(" ").map(w => w[0]).join("")}
                  </div>
                  <div>
